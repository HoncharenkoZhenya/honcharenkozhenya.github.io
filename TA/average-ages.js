...
function calculateMenAverageAge(people, century) {
  ...
  const passedMen = century ? men.filter(person => Math.ceil(person.died / 100) === century) : men;
  return passedMen.reduce((acc, person) => acc + (person.died - person.born), 0) / passedMen.length;
}
...
function calculateWomenAverageAge(people, withChildren) {
  ...
  if (withChildren) {
    const mothers = people.reduce((acc, person) => {
      acc[person.mother] = person.mother;
      return acc;
    }, {});
    women = women.filter(person => mothers[person.name]);
  }
  return women.reduce((acc, person) => acc + (person.died - person.born), 0) / women.length;
}
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  const peopleObj = people.reduce((acc, person) => ({ ...acc, [person.name]: person }), {});
  const peopleWithMother = people.filter(person => !onlyWithSon || person.sex === "m").map((person) => ({
    ...person,
    mother: peopleObj[person.mother]
  })).filter(person => person.mother);
  let countOfDeltas = 0;
  return peopleWithMother.reduce((acc, person) => acc + (person.born - person.mother.born), 0) / peopleWithMother.length;
}
